...
public class FermentableBuilderImpl implements FermentableBuilder {
    ...
    public Fermentable create() {
        ...
        if (type.equals(Fermentable.FERMENTABLE_TYPE.EXTRACT)) {
            ...
            toReturn = new LiquidFermentable(name, amount, yield, addAfterBoil,
                    origin, supplier, notes, maxInBatch, color, ibuGallonsPerPound);
            ...
            toReturn = new GrainOrAdjunctFermentable(name, type, amount, yield, color,
                    addAfterBoil, origin, supplier, notes, maxInBatch, coarseFineDiff,
                    moisture, diastaticPower, protein, recommendMash);
        } else {
            Lovibond color = new Lovibond(Double.parseDouble(colorString));
