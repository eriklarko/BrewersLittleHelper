 ...
 *
 * Taken from
 * https://www.google.se/url?sa=t&rct=j&q=&esrc=s&source=web&cd=4&ved=0CEUQFjAD
 *       &url=http%3A%2F%2Fwww.nthba.org%2Fwww%2Fdocs%2FBrew%2520Day%2520Presentation%2520-%2520Hop%2520Bittering.ppt
 *       &ei=J-x6UbGtN4mI4gSK84GgCg&usg=AFQjCNH03nlWzfx6z1ZMThbFdQaaadc_kA&bvm=bv.45645796,d.bGE&cad=rja
 *
 * IBU = (%U * Woz * %A * 7489) / (Vgal * (1 + GA)) = (%U * Wg * %A * 1000) / *
 * (Vl * (1 + GA)) where GA = (GB – 1.050)/.2 GB = (OG-1)*(Ferment Vol)/(Boil *
 * Vol) + 1
 *
 * @author thinner
 */
public class Daniels {
    ...
    public IBU calc(Iterable<HopAddition> hopAdditions, SpecificGravity originalGravity,
            Liters postBoilVolume, Liters preFermentationVolume) {
        double totalIBUs = 0;
    ...
    private double getRawIBUsFromAddition(Grams amount, Factor alphaAcids,
            SpecificGravity originalGravity, Liters postBoilVolume, Liters preFermentationVolume) {
    ...
    public IBU getIBUsFromAddition(Grams amount, Factor alphaAcids,
            SpecificGravity originalGravity, Liters postBoilVolume, Liters preFermentationVolume) {
