...
public class SimpleOriginalGravityFormula implements Formula<SpecificGravity> {
    ...
    public SpecificGravity calc(FullContext context) {
        ...
        Factor extractionEfficiency = context.extractionEfficiency.value();

        return calc(grainWeight, extractPotential, extractionEfficiency, volume);
    }
    ...
    public SpecificGravity calc(Kilograms grainWeight, ExtractPotential extractPotential, Factor extractionEfficiency, Liters volume) {
        double nominator = grainWeight.value() * extractPotential.value() * extractionEfficiency.value();
